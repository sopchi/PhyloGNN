...
class GNN(torch.nn.Module):
    def __init__(self, hidden_channels, n_hidden_convlayers):
        ...
        for k in range(n_hidden_convlayers):
            if k == 0:
                self.hidden_layers.append( GraphConv(num_edge_features, hidden_channels))
            else:
    ...
    def forward(self,edge_attr,edge_index, batch):
        ...
        for k in range(self.n_hidden_convlayers):
            edge_attr = self.hidden_layers[k](edge_attr,edge_index)
...
report = open("report_GNN_heightrelu.txt","w+")
report.write(f'leafs:{nb_leafs}\n')
report.write(f'tmax:{t_max}\n')
report.write(f'GCN_layers:{0.4*int(nb_leafs-1)}\n')
...
torch.save(model,"model_GNNhrelu") #model = torch.load('model_DNN') to load

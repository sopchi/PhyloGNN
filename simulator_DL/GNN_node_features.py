...
class GNN(torch.nn.Module):
    ...
    def forward(self,node_attr,edge_index, batch):
        ...
        node_attr = self.conv1(node_attr, edge_index)
        node_attr = node_attr.relu()
        node_attr = self.conv2(node_attr, edge_index)
        node_attr = node_attr.relu()
        node_attr = self.conv3(node_attr, edge_index)
        node_attr = node_attr.relu()
        node_attr = self.conv4(node_attr, edge_index)
...
def train():
    ...
    for k in range(n):  # Iterate in batches over the training dataset.
        ...
        if type(simu["dyck"]) != int : 
            values = simu["coo"][0]
            features = np.array(simu['node_features'])
            row = simu["coo"][1] 
            col = simu["coo"][2] 
            coo = torch.LongTensor([row,col])
            data = Data(edge_index = coo, y= torch.tensor([[p0/p2,p2-p0]]), num_nodes = 2*nb_leafs+1,edge_attr = torch.tensor([values+[0]]).T,  x=torch.tensor([features])) #[p0,1-(p2 +p0),p2]
...
def test():
    ...
    for k in range(n):  # Iterate in batches over the training dataset.
        ...
        if type(simu["dyck"]) != int: 
            ...
            col = simu["coo"][2] 
            coo = torch.LongTensor([row,col])
            data = Data(edge_index = coo, y= torch.tensor([[p0/p2,p2-p0]]), num_nodes = 2*nb_leafs+1, edge_attr = torch.tensor([values+[0]]).T, x=torch.tensor([features])) 
            out = model(data.x,data.edge_index, data.batch) # Perform a single forward pass.
